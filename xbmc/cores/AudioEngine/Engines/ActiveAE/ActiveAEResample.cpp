...
int CActiveAEResample::Resample(uint8_t **dst_buffer, int dst_samples, uint8_t **src_buffer, int src_samples, double ratio)
{
  ...
  // Example:
  // ALSA uses SNE24NER that means 24 bit load in 32 bit package and 0 dither bits
  // WASAPI uses SNE24NE which is 24 bit load in 32 bit package and 8 dither bits
  // dither bits are always assumed from the right
  // FFmpeg internally calculates with S24NE which means, that we need to shift the
  ...
  // SNE24NE 1 1 1 0 >> 8 = 0 1 1 1 = SNE24NER
  if (m_dst_fmt == AV_SAMPLE_FMT_S32 || m_dst_fmt == AV_SAMPLE_FMT_S32P)
  {
...
AVSampleFormat CActiveAEResample::GetAVSampleFormat(AEDataFormat format)
{
  ...
  else if (format == AE_FMT_S24NE4R)return AV_SAMPLE_FMT_S32;
  ...
  else if (format == AE_FMT_S24NE4RP)return AV_SAMPLE_FMT_S32P;
