...
unsigned int CFilePipe::Read(void* lpBuf, int64_t uiBufSize)
{
  ...
  return m_pipe->Read((char *)lpBuf,uiBufSize,0xFFFFFFFF);
}
...
int CFilePipe::Write(const void* lpBuf, int64_t uiBufSize)
{
  ...
  return (int)(m_pipe->Write((const char *)lpBuf,uiBufSize,0xFFFFFFFF)); // its not the size. its bool. either all was written or not.
}
